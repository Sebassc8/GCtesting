...
int GarbleBNHighMem(const GarbledCircuit& garbled_circuit, BIGNUM* p_init,
                    ...
                    bool disable_OT, NetIO *io, FerretCOT<NetIO> * ferretcot) {
...
int EvaluateBNHighMem(const GarbledCircuit& garbled_circuit, BIGNUM* p_init,
                      ...
                      NetIO *io,FerretCOT<NetIO> * ferretcot) {
  ...
  std::cout<< "Bob transfer labels time (cc) = " << ot_time << "\t(cc/bit) = "
      << ot_time
          / ((double) (garbled_circuit.e_init_size
              + (*clock_cycles) * garbled_circuit.e_input_size)) << endl;
              cout<<"Bob OT time was: "<<t<<" us"<<endl;
#if  TIMING_EVAL
	cout << ot_time << "\t";
#endif

  EvaluateHighMem(garbled_circuit, p_init, p_input, init_labels, input_labels,
                  global_key, clock_cycles, terminate_period, io,
                  output_labels, output_vals);
...
int GarbleHighMem(const GarbledCircuit& garbled_circuit, BIGNUM* p_init,
                  ...
                  int64_t terminate_period, NetIO *io, block* output_labels,
                  short* output_vals) {

io->send_block(global_key,1);
...
io->recv_block(global_key,1);
 ...
 block *wires = nullptr;
  ...
  std::cout << "Bob communication time (cc) = " << comm_time << endl;
...
int GarbleOT(const GarbledCircuit& garbled_circuit, block* init_labels,
             block* input_labels, uint64_t clock_cycles, NetIO *io,  FerretCOT<NetIO> * ferretcot) {
...
//PRG pg;
b0 = new block[message_len];
b1 = new block[message_len];
...
//FerretCOT<NetIO> * ot = new FerretCOT<NetIO>(1, 1, &io, false);

   ferretcot->send(b0, b1, message_len);
...
int EvalauteOT(const GarbledCircuit& garbled_circuit, BIGNUM* e_init,
               ...
               uint64_t clock_cycles, NetIO * io, FerretCOT<NetIO> * ferretcot) {
  uint32_t message_len = garbled_circuit.e_init_size
      + clock_cycles * garbled_circuit.e_input_size;
  bool *select = nullptr;
...
//FerretCOT<NetIO> * ferretcot = new FerretCOT<NetIO>(2, 1, &io, false);
 // IKNP<NetIO> * ot = new IKNP<NetIO>(io);
//FerretCOT<NetIO> * ot = new FerretCOT<NetIO>(2, 1, &io, false);

ferretcot->recv(message, select, message_len);
io->flush();
...
// g_init
  for (uint i = 0; i < garbled_circuit.g_init_size; i++) {
    if (i >= (uint) BN_num_bits(g_init) || BN_is_bit_set(g_init, i) == 0) {
     io->send_block(&init_labels[i * 2 + 0], sizeof(block));
    } else {
           io->send_block(&init_labels[i * 2 + 1], sizeof(block));
...
// g_input
  for (uint cid = 0; cid < clock_cycles; cid++) {
    for (uint i = 0; i < garbled_circuit.g_input_size; i++) {
     ...
     io->send_block(&input_labels[(cid * garbled_circuit.get_secret_input_size() + i)
                    * 2 + 0], sizeof(block));
      } else {
               io->send_block(&input_labels[(cid * garbled_circuit.get_secret_input_size() + i)
                    * 2 + 1], sizeof(block));   
...
// g_init
  for (uint i = 0; i < garbled_circuit.g_init_size; i++) {
   io->recv_block(&init_labels[i], sizeof(block));
...
// g_input
  for (uint cid = 0; cid < clock_cycles; cid++) {
    for (uint i = 0; i < garbled_circuit.g_input_size; i++) {
           ...
           io->recv_block(&input_labels[cid * garbled_circuit.get_secret_input_size() + i], sizeof(block));
